test_that("delete app parameter checking works", {
  ...
  expect_error(ss_deleteapp(fakesession, "notinstalledapp"),
               "App not installed on remote server")

  # Answer no when prompted
  stub(ss_deleteapp, 'utils::askYesNo', FALSE)
  expect_error( ss_deleteapp(fakesession, "fakeapp1"),
                  "Not deleting remote app")

  # Should delete when prompt = FALSE
  expect_equal(ss_deleteapp(fakesession, "fakeapp1",
                            prompt = FALSE), 0) # Return code 0 for success
})
