...
test_that("We error if host does not resolve", {
  expect_error(
    ss_connect(username = "dummy", server = "nothere.notdomain"),
...
test_that("We get an ssh session", {
  ...
  expect_s3_class(
    ss_connect(server = "mockserver", username = "mockuser"),
    "ssh"
  )
})
...
test_that("Username detection works", {
  if (requireNamespace("withr", quietly = TRUE)) {
    withr::with_envvar(
      new = c("USER" = "", "USERNAME" = "", "SHINYSENDER_USER" = ""),
      {
    ...
    withr::with_envvar(
      new = c("USER" = "alice", "USERNAME" = "", "SHINYSENDER_USER" = ""),
      {
    ...
    withr::with_envvar(
      new = c("USER" = "", "USERNAME" = "alice", "SHINYSENDER_USER" = ""),
      {
        expect_equal(getUserName(), "alice")
      }
    )
    ...
    withr::with_envvar(
      new = c("USER" = "alice", "USERNAME" = "alice", "SHINYSENDER_USER" = ""),
      {
        expect_equal(getUserName(), "alice")
      }
    )
    ...
    withr::with_envvar(
      new = c("USER" = "alice", "USERNAME" = "bob", "SHINYSENDER_USER" = ""),
      {
