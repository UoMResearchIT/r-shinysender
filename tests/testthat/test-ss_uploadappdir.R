...

{
  ...
  test_that("only direct_home method works", {
    ...
    expect_error(ss_uploadappdir(
      fakessh(),
      testapp,
      "myapp",
      method = "not_direct_home"
    ))
  })
  ...
  test_that("Remote directory detection works", {
    ...
    testapp <- create_local_shiny_app()
    expect_warning(
      ss_uploadappdir(fakessh, testapp, "myapp"),
  ...
  test_that("App case detection works", {
    ...
    expect_error(
      expect_warning(
        ss_uploadappdir(fakessh(), create_local_shiny_app(), "App1"),
        "^An app with the same name but different case exists on the server"
      ), # Warning we want
      "^Upload failed$"
    ) # Since we can't actually upload to a fake ssh connection
  })

  test_that("Proxy specification works", {
    withr::local_envvar(c(
      "SHINYSENDER_PROXY" = "http://myproxy.com:3128",
      "SHINYSENDER_PROXY_HTTP" = "http://anotherprox.com"
    ))
    expect_error(
      prepareRprofile(ShinySenderRprofilePathStaging()),
      "^If specifying SHINYSENDER_PROXY, .* must both be unset$"
    )
  })
  ...
  test_that("URL trapping works", {
    withr::local_envvar(c(
      "SHINYSENDER_PROXY_HTTP" = "xxx://anotherprox.com",
      "SHINYSENDER_PROXY_HTTPS" = "http://yetanotherprox.com"
    ))
    expect_error(
      prepareRprofile(orig_profile),
      "^Invalid proxy string for SHINYSENDER_PROXY_HTTP$"
    )
  })
  ...
  test_that("URL trapping works", {
    ...
    expect_error(
      prepareRprofile(orig_profile),
      "^Invalid proxy string for SHINYSENDER_PROXY_HTTPS$"
    )
  })
  ...
  test_that("We don't modify rprofile if no proxy", {
    ...
    expect_equal(orig_profile, prepareRprofile(orig_profile))
  })
