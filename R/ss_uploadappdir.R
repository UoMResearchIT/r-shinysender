...
ss_uploadappdir <- function(
  ...
  bundleFile <- ss_bundleapp(appDir = appDir, appName = appName, ...)
  bundleBareFile <- basename(bundleFile)

  # Check bundleBareFile has expected format since we use it in
  # shell commands
  if (
    gsub("rsconnect-bundle[^[:alnum:]]\\.tar\\.gz", "", bundleBareFile) !=
      bundleBareFile
  ) {
  ...
  if (method == "direct_home") {
    ...
    for (rd in required_dirs) {
      if (!does_directory_exist(session, rd)) {
        warning(paste0(
          "~/",
          rd,
          " does not exist. Run ss_setupserver() to create"
        ))
        return("otherError")
      }
    }
    ...
    if (!appExistsOnRemote && casecheck) {
      ...
      warning(paste(
        "An app with the same name but different case exists on the server.",
        "You may wish to delete these using ss_deleteapp():",
        casedups
      ))
    }
    ...
    tryCatch(
      {
        Sys.setenv(
          GITHUB_PAT = gitcreds::gitcreds_get(use_cache = FALSE)$password
        )
      },
      error = function(e) {
        warning(
          "Failed to retrieve GIT credentials, try gitcreds::gitcreds_set()"
        )
      }
    )
    ...
    remotecommand <- paste0(
      "set +o history && cd ./ShinyApps_staging/",
      appNameStaging,
      " && Rscript -e '",
      github_pat_insert,
      " options(renv.verbose = FALSE);",
      " lockfile <- renv:::renv_lockfile_from_manifest(\"manifest.json\");",
      # see https://github.com/rstudio/renv/issues/1667
      " lockfile$R$Version <- as.character(lockfile$R$Version);",
      " renv::lockfile_write(lockfile);",
      " renv::activate()'",
      " && Rscript -e '",
      github_pat_insert,
      "renv::restore(); renv::status()'"
    )
...
#' Custom proxy overrides (deprecated?)
#' @rdname prepareRprofile
server_proxy_overrides <- c()
...
prepareRprofile <- function(our_profile) {
  ...
  if (remote_proxy != "") {
    if (remote_proxy_http != "" || remote_proxy_https != "") {
      stop(
        "If specifying SHINYSENDER_PROXY, SHINYSENDER_PROXY_HTTPS, ",
        "and SHINYSENDER_PROXY_HTTP must both be unset"
      )
    }
  }
  ...
  if (!setproxy) {
    ...
    remote_proxy_override = server_proxy_overrides[
      names(server_proxy_overrides) == remote_server
    ]
